...
constexpr CssStyle parseInlineImpl(StringView styleStr) {
  ...
  while (it.hasNext()) {
    ...
    StringView name = decl.subString(0, eqPos).trimWhitespace();
    StringView value = decl.subString(eqPos + 1, decl.size()).trimWhitespace();
    ...
    if (name.caseCmp("text-align")) {
      ...
      if (value.endsWith("px")) {
        value = value.subString(0, value.size() - 2);
        factor = 1.0f;
...
constexpr size_t filterComments(StringView sheet, char* buffer, size_t bufferSize) {
  ...
  while (!sheet.isEmpty()) {
    ...
    StringView beforeComment = sheet.subString(0, commentStart).trimWhitespace();
...
constexpr size_t parseSheet(StringView sheet, CssRule* rule, size_t ruleCount) {
  ...
  while ((pos = sheet.findAny("@{")) != sheet.size()) {
    ...
    StringView selector = sheet.subString(0, pos).trimWhitespace();
    ...
    // Parse declarations
    StringView declarations = sheet.subString(pos + 1, endPos).trimWhitespace();
