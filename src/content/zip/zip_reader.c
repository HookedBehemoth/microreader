/*
 * zip_reader.c - Generic ZIP archive reader implementation
 *
...
/* External Arduino file wrappers */
...
struct zip_stream_context {
  ...
  size_t in_buf_ofs;
  size_t dict_ofs;
  size_t dict_read_ofs;
  size_t dict_avail;

  tinfl_status status;
  int done;
  int error;
  int uses_shared_decomp_buffer;
};
...
static zip_error read_central_directory(zip_reader* reader, zip_end_central_dir* eocd) {
  reader->file_count = eocd->total_entries;
  reader->files = (file_entry*)calloc(reader->file_count, sizeof(file_entry));
  if (!reader->files) {
    return ZIP_ERROR_OUT_OF_MEMORY;
  }
#ifdef USE_ARDUINO_FILE
  {
    char msg[128];
...
zip_error zip_open(const char* filepath, zip_reader** out_reader) {
  if (!filepath || !out_reader) {
    return ZIP_ERROR_INVALID_PARAM;
  }

  zip_reader* reader = (zip_reader*)calloc(1, sizeof(zip_reader));
  if (!reader) {
    return ZIP_ERROR_OUT_OF_MEMORY;
  }
  ...
  if (!reader->file_handle) {
    free(reader);
    return ZIP_ERROR_FILE_NOT_FOUND;
  }
  ...
  if (!find_end_central_dir(reader->file_handle, &eocd)) {
    ...
    free(reader);
    return ZIP_ERROR_NOT_A_ZIP;
  }

  zip_error err = read_central_directory(reader, &eocd);
  if (err != ZIP_OK) {
    file_close_impl(reader->file_handle);
    free(reader);
    return err;
...
zip_error zip_locate_file(zip_reader* reader, const char* filename, uint32_t* out_index) {
  ...
  return ZIP_ERROR_FILE_NOT_IN_ARCHIVE;
}

zip_error zip_extract_streaming(zip_reader* reader, uint32_t file_index, zip_data_callback callback,
  ...
  if (entry->compression == 0) {
    ...
    {
      char msg[128];
      snprintf(msg, sizeof(msg), "  [MEM] zip_extract_streaming: allocated input buffer %u bytes, Free=%d\n",
               (unsigned)chunk_size, arduino_get_free_heap());
      arduino_log_memory(msg);
    }
#else
    printf("  [MEM] zip_extract_streaming: allocated input buffer %u bytes\n", (unsigned)chunk_size);
#endif
    ...
    while (remaining > 0) {
      ...
      if (read_size == 0) {
        free(buffer);
        return ZIP_ERROR_EXTRACTION_FAILED;
      }
    ...
    {
      char msg[128];
      snprintf(msg, sizeof(msg), "  [MEM] zip_extract_streaming: attempting alloc total_size=%u\n",
               (unsigned)total_size);
      arduino_log_memory(msg);
    }
#else
    size_t total_size = sizeof(tinfl_decompressor) + chunk_size + TINFL_LZ_DICT_SIZE;
    printf("  [MEM] zip_extract_streaming: attempting alloc total_size=%u\n", (unsigned)total_size);
#endif
    ...
    {
      char msg[128];
      snprintf(msg, sizeof(msg), "  [MEM] zip_extract_streaming: allocated memory_block total_size=%u, Free=%d\n",
               (unsigned)total_size, arduino_get_free_heap());
      arduino_log_memory(msg);
    }
#else
    printf("  [MEM] zip_extract_streaming: allocated memory_block total_size=%u\n", (unsigned)total_size);
#endif
#endif
...
zip_stream_context* zip_start_streaming(zip_reader* reader, uint32_t file_index, size_t chunk_size) {
  ...
  zip_stream_context* ctx = (zip_stream_context*)calloc(1, sizeof(zip_stream_context));
  if (!ctx) {
    return NULL;
  }
#ifdef USE_ARDUINO_FILE
  {
    char msg[128];
    snprintf(msg, sizeof(msg), "  [MEM] zip_start_streaming: allocated ctx (%u bytes), Free=%d\n",
             (unsigned)sizeof(zip_stream_context), arduino_get_free_heap());
    arduino_log_memory(msg);
  }
#else
  printf("  [MEM] zip_start_streaming: allocated ctx (%u bytes)\n", (unsigned)sizeof(zip_stream_context));
#endif
  ...
  if (entry->compression == 8) {
    ...
    size_t total_size = sizeof(tinfl_decompressor) + chunk_size + TINFL_LZ_DICT_SIZE;
#ifdef USE_ARDUINO_FILE
    /* Reuse global buffer if available */
    if (g_decomp_buffer && g_decomp_buffer_size >= total_size) {
      ...
      {
        char msg[128];
        snprintf(msg, sizeof(msg), "  [MEM] zip_start_streaming: allocated decomp buffer total_size=%u, Free=%d\n",
                 (unsigned)total_size, arduino_get_free_heap());
        arduino_log_memory(msg);
      }
#else
      printf("  [MEM] zip_start_streaming: allocated decomp buffer total_size=%u\n", (unsigned)total_size);
#endif
...
int zip_read_chunk(zip_stream_context* ctx, void* buffer, size_t max_size) {
  ...
  if (ctx->entry->compression == 0) {
    ...
    /* Output any remaining data from previous decompression */
