...
void setup()
{
  ...
  // };
  // esp_vfs_fat_sdspi_mount(NULL, NULL, NULL, &mount_config, NULL);
...
void loop()
{
  ...
  if (currentButton != NONE && lastButton == NONE)
  {
    ...
    if (currentButton == CONFIRM) {
      Stopwatch sw;
      Serial.println("Loading EPUB sample...");
      auto result = Book::loadEpub("/sd/ohler.epub");
      if (result == Book::EpubLoadResult::Success) {
        Serial.println("EPUB loaded successfully!");
        auto title = Book::getTitle();
        if (title.has_value()) {
          Serial.print("Title: ");
          println(*title);
        }
      } else {
        Serial.print("Failed to load EPUB, error code: ");
        Serial.println(static_cast<int>(result));
      }
      PrintF("Took %ld microseconds to load EPUB\n", sw.elapsedMicroseconds());
